...
namespace Fodinae.Scripts.Game
{
    public class Robot : MonoBehaviour
    {
        ...
        private const float VISUALROTATIONOFFSET = -90f;

        private const float MINSMOOTHTIME = 0.05f;
        private const float MAXSMOOTHTIME = 0.15f;
        private const float REFERENCEMOVESPEED = 25f;
        ...
        {
            get => _targetAngle - VISUALROTATIONOFFSET;
            set => _targetAngle = value + VISUALROTATIONOFFSET;
        }
        ...
        protected void Update()
        {
            ...
            float speedRatio = Mathf.Clamp01(_moveSpeed / REFERENCEMOVESPEED);
            float targetSmoothTime = Mathf.Lerp(MINSMOOTHTIME, MAXSMOOTHTIME, speedRatio);
            ...
            float smoothTime = Mathf.Lerp(MINSMOOTHTIME, targetSmoothTime, distanceRatio);
        ...
        protected void OnDrawGizmos()
        {
            ...
            // Draw Rotation Arrow
            float angleRad = (transform.eulerAngles.z + VISUALROTATIONOFFSET) * Mathf.Deg2Rad;
        ...
        private class Tentacle
        {
            ...
            private const int POINTCOUNT = 5; // root + 4 segments
            private const float SMOOTHTIME = 0.08f;
            private const float MAXSEGMENTDIST = 0.2f;
            ...
            public void Update(Vector3 rootPosition, float rotationAngle, float movementFactor, float deltaTime)
            {
                ...
                {
                    // Spring movement
                    _positions[i] = Vector3.SmoothDamp(_positions[i], targetPos, ref _velocities[i], SMOOTHTIME, 50f, deltaTime);
                    ...
                    targetPos = _positions[i] + (MAXSEGMENTDIST * movementFactor * direction);
                }
            }
