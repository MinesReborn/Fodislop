...
// GetInstanceID() is deprecated in Unity 6, but GetHashCode() returns the same
// value for UnityEngine.Object subclasses and is NOT deprecated.
...
namespace Effekseer
{
	...
	public class EffekseerSystem
	{
		...
		/// <summary>
		/// Loads an effect from raw efk bytes at runtime (no .asset import required).
		/// Creates a temporary EffekseerEffectAsset, registers it, and returns it for use with PlayEffect.
		/// </summary>
		public static EffekseerEffectAsset LoadEffect(byte[] bytes, string name)
		{
			if (Instance == null)
			{
			...
			var effectAsset = ScriptableObject.CreateInstance<EffekseerEffectAsset>();
			effectAsset.efkBytes = bytes;
			effectAsset.name = name;

			Instance.LoadEffect(effectAsset);
			return effectAsset;
		}
		...
		public static EffekseerHandle PlayEffect(EffekseerEffectAsset effectAsset, EffekseerPlayEffectParameters param)
		{
			...
			IntPtr nativeEffect;
			if (Instance.nativeEffects.TryGetValue((ulong)effectAsset.GetHashCode(), out nativeEffect))
			{
		...
		private Dictionary<ulong, IntPtr> nativeEffects = new Dictionary<ulong, IntPtr>();
		private Dictionary<ulong, IntPtr> nativeResourceLoadedEffects = new Dictionary<ulong, IntPtr>();

#if UNITY_EDITOR
		// For hot reloading
		[SerializeField] private List<ulong> nativeEffectsKeys = new List<ulong>();
		...
		private void ReloadEffects()
		{
			foreach (var weakEffectAsset in EffekseerEffectAsset.enabledAssets)
			{
				...
				if (effectAsset != null)
				{
					...
					ulong id = (ulong)effectAsset.GetHashCode();
					IntPtr nativeEffect;
