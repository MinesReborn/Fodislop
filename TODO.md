...
- [ ] Здания выбиты в белое собственным альбедо, а не светом. Замер от
      03.09.2026: у `Market/0.png` **24.9 % непрозрачных пикселей ярче 240**,
      у `Teleport` 24.6 %, у `Clans` 16.2 %, у `Resp` 15.1 %, у `Up` 7.9 %.
      Верхний процентиль у всех пяти — **ровно 245.6**, одно и то же число в
      пяти разных файлах: подпись одного генератора, а не рисунка. Почти-белое
      альбедо (0.92 линейных) при освещении сцены около 2.5x даёт 2.3 линейных,
      то есть выше любой белой точки — здание белое по построению, при любой
      экспозиции и любом тонмапе. Проверено, что экспозицией не лечится: на
      -1.5 стопа здание остаётся 240/255, а земля падает со 132 до 86, то есть
      кадр только расслаивается. Блум разносит это пятно ещё на пять клеток.
      Лечится перерисовкой: у здания не должно быть площадей ярче светов сцены.
      Для сравнения, `Gun/0.png` — 0 % почти-белых и не светится.
...
- [ ] `HdrPaperWhiteScale` умножается до тонмапа, а белая точка кривой уже
      выведена из paper white: одна величина участвует дважды. Единственный
      оставшийся параметр дисплея на линейной стороне — гамма оттуда уже
      переехала в показатель возврата `ToneMapAgX`. В SDR множитель равен
      единице, поэтому на глаз не проявляется; проверять на HDR-дисплее.
