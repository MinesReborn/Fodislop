...
- [ ] Тени перенесены фильтром, но **в Unity ещё не смотрели**. С 6000.6 у UI
      Toolkit есть `filter` и `backdrop-filter`, и тринадцать свечений меню плюс
      четыре матовых стекла уехали в USS как `drop-shadow` и `blur`. Проверить
      надо две вещи: что импорт не ругается, и как это выглядит — `drop-shadow`
      тенит силуэт по альфе, а не прямоугольник коробки, и у планеты это
      размытие в 60 сигм на большой картинке каждый кадр. Если дорого, у планеты
      тень снимается первой.
- [ ] Рампа высот теней токенами. Сейчас смещения и сигмы написаны числами — так
      же, как в макете, где шкалы высот нет: она выражена самими значениями.
      Из-за этого долг «литерал в общем слое» вырос со 218 до 240. Завести
      ступени (`--elevation-1…3`) можно, но это решение о дизайне: сначала в
      макете, потом печатью в игру.
