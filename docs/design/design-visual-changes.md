...
заведён в `planning/TODO.md`.

Экрана загрузки бутстрапа **в макете не существует** — его вид выведен по
аналогии с оверлеями, а не из источника истины. Тоже в `planning/TODO.md`.
...
`dotnet run --project tools/Kern.DesignSystem -- emit-icon-textures`, а расхождение ловит
...
Теперь сверяется сказанное: `visual/kern-ui-lab/component-map.json` —
...
`dotnet run --project tools/Kern.DesignSystem -- compare-components` сравнивает правила свойство за свойством с
