...
namespace Kern.ArchitectureLinter.Rules.Assembly;

/// <summary>
/// Detects direct dependency cycles between Kern assemblies.
...
public sealed class DependencyCycleRule : IRule
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var kernAssemblies = assemblies
            .Where(a => KernAssemblyPrefixes.Any(p => a.Name.Name.StartsWith(p, StringComparison.OrdinalIgnoreCase)))
            .ToList();

        if (kernAssemblies.Count < 2)
            return Task.FromResult<IReadOnlyList<RuleViolation>>(violations);

        var graph = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);

        foreach (var assembly in kernAssemblies)
        {
            ...
            {
                if (KernAssemblyPrefixes.Any(p => reference.Name.StartsWith(p, StringComparison.OrdinalIgnoreCase)))
                {
